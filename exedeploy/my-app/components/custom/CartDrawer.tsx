...
export default function CartDrawer() {
  ...
  // Reset coupon if cart total price changes
  useEffect(() => {
  ...
  async function handleApplyCoupon(e: React.MouseEvent) {
    ...
    try {
      const res = await validateCoupon(couponCode, cart.total_price);
  ...
  async function handlePlaceOrder(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const res = await checkout(shippingAddress, phone, note, appliedCoupon?.code || undefined);
      if (res.success) {
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-1.5 border-t border-gray-100 pt-4">
                <label className="block text-[10px] font-extrabold uppercase tracking-wider text-gray-450 mb-1">
                  Mã giảm giá
                </label>
                {appliedCoupon ? (
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-xs text-gray-800 focus:border-black focus:ring-1 focus:ring-black outline-none transition uppercase"
                    />
                    <button
                      type="button"
              ...
              <div className="bg-gray-50 border border-gray-150 rounded-xl p-4.5 space-y-2.5">
                ...
                <div className="border-t border-gray-200 pt-2.5 flex justify-between text-sm font-extrabold text-gray-900">
                  ...
                  <span>{((appliedCoupon ? cart.total_price - appliedCoupon.discount_amount : cart.total_price) < 0 ? 0 : (appliedCoupon ? cart.total_price - appliedCoupon.discount_amount : cart.total_price)).toLocaleString('vi-VN')} đ</span>
                </div>
              </div>
            </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    Đặt Hàng ({((appliedCoupon ? cart.total_price - appliedCoupon.discount_amount : cart.total_price) < 0 ? 0 : (appliedCoupon ? cart.total_price - appliedCoupon.discount_amount : cart.total_price)).toLocaleString('vi-VN')} đ)
                  </>
                )}
              </button>
            </div>
